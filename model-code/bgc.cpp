...
cout << endl;
		for (int k = nFineRootOrders; k < nRootOrders; k++)
		{
                	...
                	coarseRootBiomassCarbon[j][k] = treesParams.Croot_coarse * treesParams.ar[j+3] * maximumRootBiomassCarbon[j][k]/sumMaxC * 0.96;
...
void BiogeochemicalCycles::computeMaximumRootBiomassCarbon(trees_params treesParams)
{
	...
	{
		rootDiam = treesParams.minRootDiam; //m
        	SRL = 1000*treesParams.SRL1*pow(refRootDiam, 2.0)/pow(rootDiam,2.0);
		lateralLength = treesParams.drlat[j+3]*totalRootAxialLength;
...
//how many defined cylinders fit into a hectare, m2 m-2
		...
		for (int k = 0; k < nRootOrders; ++k)
		{
			rhizCrossArea = M_PI * pow(0.5*rootDiam+0.25*rhizWidth, 2.0);
...
//Define ground area using longest lateral root length (defined in param_mod)	
	...
	groundArea = M_PI * pow(maximumLateralLength, 2.0);
...
//when N availability is less than 50% then halt stem growth
	if (N_avail_rate_plant > 0.5 && nscRatio > 1.0)
	{
		if(treesParams.useLeafModule == 1)
		{
			stem_Cfraction_upperLimit = (1/(1+treesParams.root_to_shoot ))*(1/(1+treesParams.leaf_to_stem));
		}
		else
		{
			stem_Cfraction_upperLimit = 0.2;
		}
...
cout << leafCfraction << '\t' << stemAllocation << '\t' << rgrowth << '\t' << tgrowth << '\t' << root_relative_growth << '\t' << rootAllocation << endl;
exit(1);
}
//fineRootBiomassCarbon[j][k] > maximumRootBiomassCarbon[j][k]
			...
			{
				rootCincrement = (maximumRootBiomassCarbon[j][k]-fineRootBiomassCarbon[j][k])/0.86;
			}
...
//compute plant nitrogen status
	...
	plantNstatus[0] = leafToRootCratio * leafStoredNitrogen[0]/min(getLeafNSC(), treesParams.leafNSCscalar*leafBiomassCarbon[0]);
	plantNstatus[0] += (1.0-leafToRootCratio)*getRootN()/min(getRootNSC(), treesParams.leafNSCscalar*getFineRootCarbon());
				     ...
				     trees_params& treesParams)
{
    	...
    	A = (treesParams.leafAreaMax - treesParams.initialLeafSize) / treesParams.initialLeafSize;
    	denom = pow(1.0 + A * exp(-treesParams.leafArea_Rate * thermCD), 2.0);
    	f_prime = (treesParams.leafAreaMax * A * treesParams.leafArea_Rate * 
				exp(-treesParams.leafArea_Rate * thermCD))/denom;
    	f = treesParams.leafAreaMax/(1.0 + ((treesParams.leafAreaMax - treesParams.initialLeafSize) / 
			treesParams.initialLeafSize) * exp(-treesParams.leafArea_Rate * thermCD));
				     ...
				     trees_params& treesParams)
{
    	...
    	A = (treesParams.leafAreaMax - treesParams.initialLeafSize) / treesParams.initialLeafSize;
    	denom = pow(1.0 + A * exp(-treesParams.leafArea_Rate * thermEXP), 2.0);
    	f_prime = (treesParams.leafAreaMax * A * treesParams.leafArea_Rate * 
				exp(-treesParams.leafArea_Rate * thermEXP))/denom;
    	f = treesParams.leafAreaMax/(1.0 + ((treesParams.leafAreaMax - treesParams.initialLeafSize) / 
			treesParams.initialLeafSize) * exp(-treesParams.leafArea_Rate * thermEXP));
...
// new function, informed by Marc Brock's Timmer experiment data from 2016
...
double BiogeochemicalCycles::calcHydRER(double hydraulicStatus)
{
    	return(1.0); // k_p_e.latK[1]/ksat[1][1];
}
...
// placeholder- N constraint on RAR will go here
            		...
            		if ((SingleLeafAreaPotential[k] + deltaApot) > treesParams.leafAreaMax)

// if the change will bring total greater than max
            		{ 
                		deltaApot =  treesParams.leafAreaMax - SingleLeafAreaPotential[k];
            		}
...
// ------------------------------
        	...
        	if (SingleLeafThermTm[k] > ((1.0-treesParams.proportion_CD)* duration_Leaf))
        	{
            		...
            		RER = calcRER(thermEXP, treesParams); // theoretical optimal RER

// if current ts is first expansion step
            		if (SingleLeafArea[k] == 0.0) 
            		{
                		SingleLeafArea[k] = treesParams.initialLeafSize; // cm2
...
// placeholder- hydraulic status effect on RER
            		water_RER_coef = calcHydRER(1.0); 
...
// update costs
            		...
            		//cout << "cost of exp, kgC/ha, = " << delta_nsc << endl;
...
// maximum projected ground area (denominator of LAI)
    	max_ProjArea = 4.0*treesParams.leaf_len_to_width * treesParams.leafAreaMax * 
